...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # Declare embedding to hidden weight and bias.
        self.embed_to_hidden_weight = nn.init.xavier_uniform(nn.Parameter(torch.empty(self.n_features * self.embed_size, hidden_size))) # W 
        self.embed_to_hidden_bias = nn.init.uniform_(nn.Parameter(torch.empty(self.hidden_size))) # b1
        
        # Construct `self.dropout` layer.
        self.dropout = nn.Dropout(self.dropout_prob) 

        # Construct Logits (output) Layer.
        self.hidden_to_logits = nn.Linear(self.hidden_size, self.n_classes)
        # Declare hidden to logits weight and bias. 
        self.hidden_to_logits_weight = nn.init.xavier_uniform_(nn.Parameter(torch.empty(self.hidden_size, self.n_classes))) # U
        self.hidden_to_logits_bias = nn.init.uniform_(nn.Parameter(torch.empty(self.n_classes))) # b2
    ...
    def embedding_lookup(self, w):
        ...
        # Concatenate E1, E2, ...Em into E1; E2; ... :Em 
        # Flattens input by reshaping it into a one-dimensional tensor.
        x = self.embeddings[w]
        x = x.view(x.shape[0], -1) # Concatenate it and make it in one dimention. 
