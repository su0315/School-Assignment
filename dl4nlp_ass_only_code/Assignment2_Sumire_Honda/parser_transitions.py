...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "LA":
            dependency.append(self.stack[-2])
            self.stack.remove(self.stack[-2])
            dependency.insert(0, self.stack[-1])# Put the first last item on the left most side. 
            self.dependencies.append(tuple(dependency))

        if transition == "RA":
            dependency.append(self.stack[-1])
...
def minibatch_parse(sentences, model, batch_size):
    ...
    for sentence in sentences:
        pp = PartialParse(sentence)# Initialize the class PartialParse
        partial_parses.append(pp) # Make list of PartialParses.
    unfinished_parses = partial_parses[:] # Make shallow copy of partial_parses
    
    while len(unfinished_parses) > 0:
        batch = unfinished_parses[:batch_size]# Split unfinished_parses in batch_size.
        transitions = model.predict(batch)

        for i in range(len(batch)): # Put index to every parse within each batch.
            batch[i].parse_step(transitions[i]) # Perform parse_step on each parse with its transition. 

            if len(batch[i].buffer) == 0 and len(batch[i].stack) == 1: # Check if each parse is completed. 
                unfinished_parses.remove(batch[i]) # Remove the completed parse from unfinished_parses.

    for partial_parse in partial_parses: # Check pp in partial_parses.
        dependencies.append(partial_parse.dependencies) # Collect each dependency in dependencies list 
